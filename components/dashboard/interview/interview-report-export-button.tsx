...
export function InterviewReportExportButton({ sessionId }: { sessionId: string }) {
  ...
  async function handleExport(format: ExportFormat) {
    ...
    try {
      const response = await fetch(
        `/api/interviews/${sessionId}/report/export?format=${format}`
      );
      if (!response.ok) {
      ...
      const filename = match?.[1] ?? `interview-report.${format === "pdf" ? "pdf" : "md"}`;
