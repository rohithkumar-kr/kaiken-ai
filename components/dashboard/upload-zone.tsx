...
function getFileType(file: File): "pdf" | "docx" {
...
function validateFile(file: File): string | null {
  if (!ACCEPTED_MIME_TYPES.has(file.type) && !/\.(pdf|docx)$/i.test(file.name)) {
...
export function UploadZone({ resume }: { resume?: ExistingResume | null }) {
  ...
  async function runAnalyze(payload: AnalyzePayload, startStep: number, toastId?: string | number) {
    ...
    tickerRef.current = window.setInterval(() => {
      ...
      setStepIndex(Math.min(startStep + Math.floor(elapsed / STEP_DURATION_MS), STEPS.length - 1));
    }, 200);
    ...
    try {
      ...
      if (!response.ok || !data.parsed) {
        throw new Error(data.error ?? "Failed to analyze your resume");
      }
    ...
    void runAnalyze(
      {
        ...
        fileType: resume.fileType === "pdf" ? "pdf" : "docx",
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-muted-foreground truncate text-xs">
                {fileInfo?.name} — AI extraction complete and saved to your account.
              </p>
            </div>
          </div>
      ...
      {resume?.parseStatus === "FAILED" && phase === "idle" ? (
        <div className="border-destructive/30 bg-destructive/5 flex items-center justify-between gap-4 rounded-xl border px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">Previous analysis of {resume.fileName} failed</p>
              <p className="text-muted-foreground text-xs">
                The file is still on file. Retry parsing without re-uploading.
              </p>
            </div>
          </div>
            ...
            return (
              <li
                ...
                className={cn(
                  ...
                  isDone && "bg-muted/40 text-muted-foreground border-transparent",
                  isActive && "border-foreground/20 bg-muted/40"
                )}
              >
                {isDone ? (
                ...
                {isActive && phase === "uploading" && stepIndex === 0 ? (
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">
                ...
                {fileInfo ? (
                  <>
                    {" "}
                    <FileText className="inline size-3.5" aria-hidden="true" /> {fileInfo.name},{" "}
